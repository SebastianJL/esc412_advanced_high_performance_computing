...
int main(int argc, char *argv[]) {
    ...
    ptrdiff_t v = fftw_mpi_local_size_many(sizeof(n) / sizeof(n[0]), n, howmany,
                                           FFTW_MPI_DEFAULT_BLOCK,
                                           MPI_COMM_WORLD, &n0, &s0);
    printf("r: %d: start %llu count %llu number %llu\n", iRank, s0, n0, v);
//    v = fftw_mpi_local_size_many_transposed(
//        sizeof(n) / sizeof(n[0]), n, howmany, FFTW_MPI_DEFAULT_BLOCK,
//        FFTW_MPI_DEFAULT_BLOCK, MPI_COMM_WORLD, &n0, &s0, &n1, &s1);
//    printf("k: %d: start %llu,%llu count %llu,%llu number %llu\n", iRank, s0,
//           s1, n0, n1, v);
    Array<complex<double>, 3> grid(Range(s0, s0 + n0 - 1), Range(0, nGrid - 1),
                                   Range(0, nGrid / 2));
    ...
    if (iRank == 2)
        cout << grid(Range::all(), Range::all(), 0) << endl;
    auto plan = fftw_mpi_plan_many_transpose(
        nGrid, nGrid, 2 * (nGrid / 2 + 1), FFTW_MPI_DEFAULT_BLOCK,
